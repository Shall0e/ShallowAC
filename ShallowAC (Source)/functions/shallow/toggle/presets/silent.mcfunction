execute @s[tag=1owner] ~ ~ ~ scoreboard players set anticbe togglecbe 4
execute @s[tag=1owner] ~ ~ ~ scoreboard players set antiitem toggleitem 2
...
execute @s[tag=1owner] ~ ~ ~ tellraw @a {"rawtext":[{"text":"⟫ §c"},{"selector":"@s"},{"text":" has set the §6Toggle-Preset§c to §6Silent§c. §e(ShallowAC)"}]}
tag @s add debug
