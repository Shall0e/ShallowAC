execute @s[tag=1owner] ~ ~ ~ scoreboard players set anticbe togglecbe 0
...
execute @s[tag=1owner] ~ ~ ~ scoreboard players set antilag togglelag 0
execute @s[tag=1owner] ~ ~ ~ tellraw @a {"rawtext":[{"text":"⟫ §c"},{"selector":"@s"},{"text":" has set the §6Toggle-Preset§c to §6Anarchy§c. §e(ShallowAC)"}]}
tag @s add debug
