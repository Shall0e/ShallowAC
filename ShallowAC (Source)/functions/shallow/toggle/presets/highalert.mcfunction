...
execute @s[tag=1owner] ~ ~ ~ scoreboard players set antifly togglefly 1
...
execute @s[tag=1owner] ~ ~ ~ tellraw @a {"rawtext":[{"text":"⟫ §c"},{"selector":"@s"},{"text":" has set the §6Toggle-Preset§c to §6High-Alert§c. §e(ShallowAC)"}]}
tag @s add debug
